...
- **api:** include `generator_version` and `generator_capabilities` in `/api/entity` responses for mud-server adapter parity.

### Bug Fixes

- **axis-policy:** align canonical label ordering for `physique` and `health`, and normalize occupation visibility label spelling to `discrete`.
- **versioning:** sync package `__version__` with project version `0.11.1`.

### Documentation

- **docs:** add condition-axis contract parity release notes in `docs/release_notes.rst` and link from docs index.
...
- **packaging:** exclude **pycache** from distribution ([#6](https://github.com/pipe-works/pipeworks_entity_state_generation/issues/6)) ([dd9763b](https://github.com/pipe-works/pipeworks_entity_state_generation/commit/dd9763b1d65e0e3fbdb85474f804f5a19357f80e))
